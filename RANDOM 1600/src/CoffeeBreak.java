...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            int n= ni(), m= ni(), d= ni()+1;
            int[] a= ni(n);

            TreeSet<int[]> tset = new TreeSet<>((i, j)->(i[0]== j[0]? i[1]- j[1]: i[0]- j[0]));
            for (int i = 0; i < n; i++) tset.add(new int[]{a[i], i});

            int days= 0;
            int[] ans= new int[n];
            while(!tset.isEmpty()) {
                int[] curr= tset.pollFirst();
                ans[curr[1]]= days+1;

                while(curr[0]+ d<= m) {
                    if(tset.ceiling(new int[]{curr[0]+d , 0})!= null) {
                        curr= tset.ceiling(new int[]{curr[0]+d, 0});
                        tset.remove(curr);
                        ans[curr[1]]= days+1;
                    }
                    else break;
