...
//        t = ni()/
        for (int ii = 0; ii < t; ii++) {
            ...
            while(true) {
                ...
                int n= arr.length; if(n==1) break;
                for(int i=0;i<n;i++) {
                    if(arr[i]== '\u0000') continue;
                    if(i!= 0 && i!= n-1 && (Math.abs(arr[i]- arr[i-1])==1 || Math.abs(arr[i]- arr[i+1])==1))
                        delete.add(new int[]{arr[i], i});
                    else if(i== 0 && Math.abs(arr[i]- arr[i+1])==1) delete.add(new int[]{arr[i], i});
                    else if(i== n-1 && Math.abs(arr[i]- arr[i-1])==1) delete.add(new int[]{arr[i], i});
                }

                if(delete.size()== 0) break;
                Collections.sort(delete, (a, b)-> (b[0]== a[0]? a[1]- b[1]: b[0]- a[0]));
                arr[delete.get(0)[1]]= '\u0000';

                int c= 0;
                char[] _arr= new char[arr.length-1];
                for(int i=0;i<arr.length;i++) if(arr[i]!= '\u0000') _arr[c++]= arr[i];
                arr= _arr;
//                out.println(new String(arr));
            }

            out.println(_n- arr.length);
        }
    }

    public static void main(String[] args) throws Exception {
        new RemoveAdjacent().run();
    }
