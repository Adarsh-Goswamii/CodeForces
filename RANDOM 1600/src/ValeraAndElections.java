...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for (int i = 0; i < n-1; i++) {
                ...
                arr[u].add(new int[]{v, type});
                arr[v].add(new int[]{u, type});
            }

            ans= new ArrayList<>();
            out.println(dfs(arr, 1, -1, 1));
    ...
    List<Integer> ans;
    private int dfs(List<int[]>[] arr, int root, int par, int type) {
        int ret= 0;
        for(int[] i: arr[root]) {
            if(i[0]== par) continue;
            ret+= dfs(arr, i[0], root, i[1]);
        }

        if(type== 2) {
            if(ret== 0) ans.add(root);
