...
public class RGBSubstring2 {
    ...
    void solve() throws Exception {
        ...
        for (int ii = 0; ii < t; ii++) {
            ...
            int ans= Math.min(helper(arr, "RGB".toCharArray(), k), helper(arr, "GBR".toCharArray(), k));
            ans= Math.min(ans, helper(arr, "BRG".toCharArray(), k));
            out.println(ans);
        }
    }

    private int helper(char[] arr, char[] infinite, int k) {
        Deque<Integer> q= new ArrayDeque<>();
        for (int i = 0; i < k; i++) {
            if(infinite[i%3]!= arr[i]) q.addLast(i);
        }
        ...
        for (int i = k; i < arr.length; i++) {
            ...
            if(infinite[i%3]!= arr[i]) q.addLast(i);
            ret= Math.min(ret, q.size());
        }
        return ret;
    }

    public static void main(String[] args) throws Exception {
        new RGBSubstring2().run();
    }
