...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            int max_len= 0, start= 0, last= 0;
            while(last!= n-1) {
                int mistake= 0, check= -1;
                while(last!= n-1 && mistake!= 2) {
                    if(a[last]>= a[last+1]) {
                        if(last!= 0 && a[last-1]+1== a[last+1]) { check= last; break; }
                        else {
                            if(mistake== 0) check= last+1;
                            a[last]= last== 0? -1: a[last-1]+1;
                            mistake++;
                        }
                    }
                    last++;
                ...
                out.println(start+" "+last);
                max_len= Math.max(max_len, last- start+ (last==n-1? 1: 0));
                start= check;
            }
