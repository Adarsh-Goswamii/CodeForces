...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for (int i = 0; i < n; i++) {
                if(map.containsKey(color[i])) map.get(color[i]).add(i+1);
                else {
                    List<Integer> temp= new ArrayList<>();
                    temp.add(i+1);
                    map.put(color[i], temp);
                }
            }

            int ans= 0, max=-1;
            for(int i: map.keySet()) {
                Set<Integer> diversity= new HashSet<>(); diversity.add(i);
                for(int j: map.get(i)) {
                    for(int k: arr[j])
                    diversity.add(color[k-1]);
                }

                if(diversity.size()> max) {
                    max= diversity.size();
                    ans= i;
                }
                else if(diversity.size()== max) ans= Math.min(ans, i);
            }
