...
public class C {
    ...
    void solve() throws Exception {
        ...
        for (int ii = 0; ii < t; ii++) {
            int n = ni();
            long[][] a = new long[n + 1][];
            for (int i = 1; i <= n; i++) a[i] = new long[]{ni(), ni()};
            ...
            out.println(Math.max(helper(arr, a, root, 0, -1),
                    helper(arr, a, root, 1, -1)));
        }
    }

    long[][] dp;
    private long helper(List<List<Integer>> arr, long[][] a, int root, int val, int par) {
        if (dp[root][val] != -1) return dp[root][val];
        else {
            long ret = 0l, ret2= 0l;
            for (int i : arr.get(root)) {
                if (par == i) continue;
                ret += helper(arr, a, i, 0, root);
                ret2 += helper(arr, a, i, 1, root);
            }

            if(par== -1) return Math.max(ret, ret2);
            ret+= Math.abs(a[par][val] - a[root][0]);
            ret2+= Math.abs(a[par][val] - a[root][1]);
            return dp[root][val]= Math.max(ret, ret2);
        }
    }
