...
public class B {
    ...
    void solve() throws Exception {
        ...
        for (int ii = 0; ii < t; ii++) {
            ...
            int minp= imax, maxn= imin;
            for(int i: a) {
                if(i> 0) minp= Math.min(minp, i);
                else maxn= Math.max(maxn, i);
            }
            ...
            if(minp!= imax) {
                ...
                for(int i=0;i<n;i++) {
                    ...
                    if(i!= 0 && i!= n-1) {
                        if(Math.abs(a[i-1]- a[i]) >= minp && Math.abs(a[i+1]- a[i])>= minp)
                        { c++; set.add(a[i]); }
                    }
                    else if(i== 0) {
                        if(Math.abs(a[i+1]- a[i])>= minp) { c++; set.add(a[i]); }
                    }
                    else if(i== n-1) {
                        if(Math.abs(a[i-1]- a[i]) >= minp) { c++; set.add(a[i]); }
                    }
