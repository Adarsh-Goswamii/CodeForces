...
public class C {
    ...
    void solve() throws Exception {
        ...
        for (int ii = 0; ii < t; ii++) {
            ...
            long ans= 0l;
            for (int i = 0; i < n; i++) {
                long start= l- a[i], last= r- a[i];
                int val0= bin1(a, i+1, n-1, start);
                int val1= bin2(a, i+1, n-1, last);

                if(val0== -1 || val1== -1) continue;
                else if( a[val0]< start || a[val0]> last) continue;
                else if( a[val1]< start || a[val1]> last) continue;
                else {
                    ans+= Math.abs(val1- val0)+ 1;
                }
            }
            out.println(ans);
        }
    ...
    // returns the smallest value greater than or equal to find
    private int bin1(int[] a, int s, int l, long find) {
        int ret= -1;
        while(s<= l) {
            int m= s+(l-s)/2;
    ...
    private int bin2(int[] a, int s, int l, long find) {
        int ret= -1;
        while(s<= l) {
            int m= s+(l-s)/2;
