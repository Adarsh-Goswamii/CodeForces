...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for (int i = 0; i < n; i++) ans[i][i+1]= i;
            for (int i = n-1; i>=0; i--) fill(ans, a, i);

            for (int i = 0; i < n; i++) {
                for (int j = 0; j <=i; j++) out.print(ans[i][j]+" ");
                out.println();
            }
        }
    }

    private void fill(int[][] ans, int[] a, int ind) {
        int val= a[ind], row= ind;
        while(val!= 0) {
            // check for index of prev row
            int j= row< ans.length-1? ans[row+1][row+2]: 0;

            // fill till that in this row
            int i= ans[row][row+1];
            while(val!= 0 && i>= j) {
                ans[row][i--]= a[ind];
                val--;
