...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            int n= ni();
            int[][] a= new int[n][n];
            for (int i = 0; i < n; i++) a[i]= ni(n);

            if(n== 1) { out.println(1); return; }

            Set<Long> contains= new HashSet<>();
            Set<Long> ncontains= new HashSet<>();
            for (int i = 0; i < n; i++) {
                long sum= 0l;
                boolean b= false;
                for (int j = 0; j < n; j++) { sum+= a[i][j]; if(a[i][j]== 0) b= true; }
                if(b) contains.add(sum);
                else ncontains.add(sum);
            }

            for (int i = 0; i < n; i++) {
                long sum= 0l;
                boolean b= false;
                for (int j = 0; j < n; j++) { sum+= a[j][i]; if(a[j][i]== 0) b= true; }
                if(b) contains.add(sum);
                else ncontains.add(sum);
            }
            ...
            boolean b= false;
            long sum= 0l;
            for (int i = 0; i < n; i++) { if(a[i][i]== 0) b= true; sum+= a[i][i]; }
            if(b) contains.add(sum);
            else ncontains.add(sum);

            sum= 0l;
            b= false;
            for (int i = 0; i < n; i++) { if(a[i][n-i-1]== 0) b= true; sum+= a[i][n-1-i]; }
            if(b) contains.add(sum);
            else ncontains.add(sum);

            if(ncontains.size()>1 || contains.size()>1) out.println(-1);
            else { for(long i: ncontains) for(long j: contains) if(i>j) out.println(i-j); else out.println(-1); }
        }
    }

    public static void main(String[] args) throws Exception {
        new ChrisAndMagicSquare().run();
    }
