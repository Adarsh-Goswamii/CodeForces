...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for(int i= 0;i<n;i++) {
                if(val+ a[i]<= h) {
                    while(i< n && val+ a[i]<= h) { val+= a[i]; i++; }
                    i--;
                    val= Math.max(0, val- k);
                    time++;
                } else {
                    long temp= (long)Math.ceil((val+a[i]- h)/(1.0d*k));
                    time+= temp;
                    val= Math.max(val- temp*k, 0);
                    i--;
                }
//                out.println(val+" "+time);
            }

            out.println(time+ (long)Math.ceil(val/(1.0d*k)));
        }
    }

    public static void main(String[] args) throws Exception {
        new VanyaAndFoodProcessor().run();
    }
