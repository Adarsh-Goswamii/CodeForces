...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for (int i = 0; i < q; i++) b[i]= nl();

            long[] prefix= new long[n];
            for (int i = 0; i < n; i++) prefix[i]= a[i]+ (i!= 0? prefix[i-1]: 0l);

            long sum= 0l;
            for (int i = 0; i < q; i++) {
                sum+= b[i];
                int ind= bin(prefix, sum);
                if(ind== n-1) { out.println(n); sum= 0l; }
                else out.println(n-ind-1);
            }
        }
    }

    // returns index of the greatest smaller no or equal to given sum.
    private int bin(long[] prefix, long sum) {
        int s= 0, l= prefix.length-1, ret= -1;
        while(s<= l) {
            int m= s+(l-s)/2;
