...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            int n= ni(), k= ni();
            List<Integer>[] map= new ArrayList[5001];
            for (int i = 0; i < 5001; i++) map[i]= new ArrayList<>();

            int[] a= ni(n);
            int max= 0;
            for(int i=0;i<n;i++) {
                map[a[i]].add(i);
                max= Math.max(max, map[a[i]].size());
            }
            ...
            if(max> k) out.println("NO");
                ...
                for(int i=0;i<=5000;i++) {
                    for(int ind: map[i]) { a[ind]= color+1; color= (color+1)%k; }
                }
                print(a);
            }
        }
