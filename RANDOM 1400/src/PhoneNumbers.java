...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                for (int j = 0; j < val; j++) {
                    String no= nw();
                    if(!value.contains(no)) value.add(no);
                }
            ...
            for (String s: map.keySet()) {
                ...
                Collections.sort(value, (a, b)-> (b.length()- a.length()));
                suffixCheck(value);
                out.print(s+" "+value.size()+" ");
                for(String ss: value) out.print(ss+" ");
                out.println();
            }
        }
    }

    private void suffixCheck(List<String> str) {
        List<String> remove = new ArrayList<>();
        for (int i = 0; i < str.size(); i++) {
            for(int j= 0;j<i;j++) {
                String l= str.get(j), s= str.get(i);
                if(l.substring(l.length()- s.length()).equals(s)) remove.add(s);
            }
        }

        for(String s: remove) str.remove(s);
    }
