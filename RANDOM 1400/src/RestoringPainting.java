...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            long ans= 0l;
            for (int i = 1; i <= n; i++) if(check(a, b, c, d, i, n)) ans+= n;
            out.println(ans);
        }
    }

    private boolean check(int a, int b, int c, int d, int e, int n) {
        int h= e+ a- d;
        int f= e+ b- c;
        int i= a+ e+ b- c- d;

        if(a+ f!= d+ i) return false;
        if(a+ f+ c!= b+ h+ d) return false;
        if(b+ h!= c+ i) return false;
        if(h<= 0 || f<= 0 || i<= 0) return false;
        if(h> n || f> n || i> n) return false;
