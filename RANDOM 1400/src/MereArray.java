...
public class MereArray {
    ...
    void solve() throws Exception {
        ...
        for (int ii = 0; ii < t; ii++) {
            ...
            for(int i=0;i<n;i++) if(a[i]%min== 0) set.add(i);

            List<Integer> sort = new ArrayList<>();
            for (int i = 0; i < n; i++) sort.add(i);
            Collections.sort(sort, (_a, _b)->(a[_a]- a[_b]));

            boolean ans= true;
            for (int i = 0; i < n && ans; i++) {
                if((set.contains(i) && set.contains(sort.get(i))) || (i== sort.get(i))) continue;
                else ans= false;
            }
