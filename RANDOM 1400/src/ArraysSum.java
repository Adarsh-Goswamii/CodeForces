...
public class ArraysSum {
    ...
    void solve() throws Exception {
        ...
        for (int ii = 0; ii < t; ii++) {
            ...
            if(k== 1 && set.size()> 1) out.println(-1);
            else if(set.size()<= k) out.println(1);
            else out.println((int)Math.ceil((set.size()-k)/(k-1.0d))+ 1);
        }
    }

//    boolean isSubsetSum(List<Integer> set, int n, int sum)
//    {
...
//                if (j >= set.get(i-1))
//                    subset[i][j] = subset[i - 1][j] || subset[i - 1][j - set.get(i-1)];
//            }
//        }
//
//        return subset[n][sum];
