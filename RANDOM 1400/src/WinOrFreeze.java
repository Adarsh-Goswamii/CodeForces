...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            long q= nl();
            // player who cannot make a move wins.

            Map<Long, Integer> map= primeFactorization(q);
            int val= 0;
            for(int i: map.values()) val+= i;

            if(val== 1) out.println(1+"\n"+0); // no move can be made.
            else if(val== 2) out.println(2);
            else {
                out.println(1);

                int count= 0;
                long ans= 1l;
                outer: for(Map.Entry<Long, Integer> i: map.entrySet()) {
