...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            long sum= 0l;
            for(int i: a) sum+= (int)Math.ceil(i/2d);

            if(w< sum) out.println(-1);
            else {
                w-= sum;
                TreeSet<int[]> tset = new TreeSet<>((i, j)->
                        (i[1]== j[1]? i[2]== j[2]? i[0]- j[0]: i[2]- j[2]: i[1]- j[1]));
                for (int i = 0; i < n; i++) tset.add(new int[]{i, a[i]-= (int) Math.ceil(a[i]/2d), arr[i]});

                while(!tset.isEmpty() && w> 0) {
                    int[] curr= tset.pollLast();
                    if(curr[1]> w) { a[curr[0]]-= w; w= 0; }
                    else { w-= a[curr[0]]; a[curr[0]]= 0; }
                }

                for (int i = 0; i < n; i++) out.print((arr[i]- a[i])+" ");
                out.println();
            }
