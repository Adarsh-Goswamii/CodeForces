...
//        t = ni();
    ...
    private boolean check(String a, String b, int len) {
        String divisor= a.substring(0, len);

        int i= 0;
        while(i<a.length()) {
            if(!a.substring(i, i+ len).equals(divisor)) return false;
            i+= len;
        }

        i= 0;
        while(i<b.length()) {
            if(!b.substring(i, i+ len).equals(divisor)) return false;
            i+= len;
        }
    ...
    private void commonDiv(int n, int m, List<Integer> list) {
