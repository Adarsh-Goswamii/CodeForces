...
public class SuffixOperation {
    ...
    void solve() throws Exception {
        ...
        for (int ii = 0; ii < t; ii++) {
            ...
            long oper= 0l, max= Math.max(Math.abs(arr[0]-arr[1]), Math.abs(arr[n-1]- arr[n-2]));
            for (int i = 1; i < n; i++) {
                oper+= Math.abs(arr[i]- arr[i-1]);
                if(i!= n-1) {
                    long temp= Math.abs(arr[i-1]- arr[i])+ Math.abs(arr[i]- arr[i+1]);
                    max= Math.max(max, temp- Math.abs(arr[i+1]- arr[i-1]));
                }
            }

            out.println(oper- max);
        }
    }

    public static void main(String[] args) throws Exception {
        new SuffixOperation().run();
    }
