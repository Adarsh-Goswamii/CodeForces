...
//        t = ni();
    ...
    private boolean predicate(char[] arr, int m) {
        List<Integer>[] map= new List[26];
        for (int i = 0; i < 26; i++) map[i]= new ArrayList<>();

        for (int i = 0; i < 26; i++) map[i].add(-1);
        for (int i = 0; i < arr.length; i++) map[arr[i]-'a'].add(i);
        for (int i = 0; i < 26; i++) map[i].add(arr.length);

        outer: for (int i = 0; i < 26; i++) {
            for(int j=0;j<map[i].size()-1;j++) {
                if(map[i].get(j+1)-map[i].get(j)<= m) continue;
                else continue outer;
            }
            return true;
        }
