...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            ans= 0;
            helper(a, 0, new char[10], 0);

            if(ans!= 0) out.println(a.length- ans);
            else out.println(-1);
        }
    }

    int ans;
    private void helper(char[] a, int i, char[] arr, int j) {
        if(i== a.length) {
            StringBuilder num= new StringBuilder();
            for(int k=0;k<arr.length;k++) {
                if(arr[k]== '\u0000') break;
                else num.append(arr[k]);
            }

            if(num.length()== 0 || num.charAt(0)== '0') return;
//            out.println(num);
            int val= Integer.parseInt(num.toString());
            if((int)Math.sqrt(val)*(int)Math.sqrt(val)== val) ans= Math.max(ans, num.length());
        }
        else {
            helper(a, i+1, arr, j);
            arr[j]= a[i];
            helper(a, i+1, arr, j+1);
            arr[j]= '\u0000';
        }
    }
