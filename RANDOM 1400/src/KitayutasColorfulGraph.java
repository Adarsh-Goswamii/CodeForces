...
//        t = ni();
    ...
    Set<Integer> colors;
    int u;
    private void dfs(List<List<int[]>> a, int u, int v, boolean[] seen, int c) {
        if(seen[u]) return;
        else if(u== v) colors.add(c);
        else {
            seen[u]= true;
            for(int[] i: a.get(u)) {
                if(i[1]!= c && u!= this.u) continue;

                dfs(a, i[0], v, seen, i[1]);
            }
            seen[u]= false;
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        new KitayutasColorfulGraph().run();
    }
