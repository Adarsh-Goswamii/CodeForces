...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            List<Integer> neg = new ArrayList<>(); neg.add(-1);
            for (int i = 0; i < n; i++) if(a[i]< 0) neg.add(i);
            neg.add(n);

            int[] pre= new int[neg.size()];
            for (int i = pre.length-2; i> 0; i-=2)
                pre[i]= (i+2< pre.length-1? pre[i+2]: 0)+ neg.get(i+1)- neg.get(i);

            for (int i = pre.length-3; i> 0; i-=2)
                pre[i]= (i+2< pre.length-1? pre[i+2]: 0)+ neg.get(i+1)- neg.get(i);

            long ans= 0l;
            for (int i = 0; i < n; i++) {
                int ind= bin(neg, 1, neg.size()-2, i);
                if(a[i]> 0) {
                    if(ind== -1) ans+= n-i;
                    else if(ind+1== neg.size()-1) ans+= neg.get(ind)- i;
                    else ans+= pre[ind+1]+ neg.get(ind)- i;
                }
                else {
                    if(ind!= -1) ans+= pre[ind];
                }

//                out.println(i+" "+(ans- temp));
            }

            out.println((n*1l*(n+1)/2-ans)+" "+ans);
        }
    }

    private int bin(List<Integer> arr, int s, int l, int f) {
        int ret= -1;
        while(s<= l) {
            int m= s+ (l-s)/2;

            if(arr.get(m)> f) { l= m-1; ret= m; }
            else s= m+1;
        }
