...
public class ApproximatingAConstantRange {
    ...
    /**
     * even if we have no relation between the elements, this logic will work just fine.
     */
...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for(int i=0;i<n;i++) {
                ...
                int in= min.peekFirst(), ix= max.peekFirst();
                if(a[ix]- a[in]<= 1)
                    ans= Math.max(ans, i- Math.min(ix, in)+1);
                else {
                    while(!min.isEmpty() && !max.isEmpty()) {
                        in= min.peekFirst(); ix= max.peekFirst();
                        if(a[ix]- a[in]<= 1) break;
                        if(in< ix) min.pollFirst();
                        else max.pollFirst();
                    }
                    ans= Math.max(ans, i- Math.min(ix, in)+1);
                }
    ...
    public static void main(String[] args) throws Exception {
        new ApproximatingAConstantRange().run();
    }
