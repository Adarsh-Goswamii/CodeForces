...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for (int i = 1; i < n+1; i++) arr[i]= ni();

            int[][] oper= new int[m+1][3];
            for (int i = 1; i < m+1; i++) oper[i]= new int[]{ni(), ni(), ni()};

            int[] r= new int[m+2];
            for (int i = 0; i < k; i++) {
                int start= ni(), last= ni();
                r[start]+= 1;
                r[last+1]+= -1;
            }
            ...
            for(int i=1;i<=m;i++) {
                count+= r[i];
                int start= oper[i][0], last= oper[i][1], d= oper[i][2];
                add[start]+= d*1l*count;
                add[last+1]+= -1l*d*count;
            }
            ...
            for(int i=0;i<arr.length;i++) arr[i]+= add[i];
            print(arr);
        }
    }

    public static void main(String[] args) throws Exception {
        new GregAndArray().run();
    }
