...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            long ans= 0l, max= 0l, total= 0l, extra= 0l;
//            out.println(map+" \n"+list);
            for(int i: list) {
                total+= i*1l*map.get(i);
//                ans+= Math.max(map.get(i), i- max);
                if(map.get(i)> i- max) {
                    ans+= Math.max(map.get(i)- extra, i- max);
                    extra= Math.max(0, extra- (map.get(i)- i+max));
                }
                else {
                    ans+= i- max;
                    extra+= i- max- map.get(i);
                }
