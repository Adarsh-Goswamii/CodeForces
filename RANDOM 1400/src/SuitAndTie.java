...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for (int i = 0; i < a.length; i++) {
                if(map.containsKey(a[i])) map.get(a[i])[1]= i;
                else map.put(a[i], new int[]{i, 0});
            }

            long ans= 0l;
            for(int i: map.keySet()) {
                boolean left= helper(map, a, map.get(i));
                if(left) swap(a, map.get(i)[1]-1, map.get(i)[1]);
                else swap(a, map.get(i)[0], map.get(i)[0]+1);
                ans+= Math.abs(map.get(i)[0]- map.get(i)[1]);
            }
            out.println(ans);
        }
    }
    ...
    private boolean helper(Map<Integer, int[]> map, int[] a, int[] val) {
        int s= val[0]+1, l= val[1];
        int left= 0, right= 0;
        ...
        while(s<= l) {
            ...
            if(temp[1]> val[1]) right++;
            else if(temp[0]< val[0]) left++;
        }
