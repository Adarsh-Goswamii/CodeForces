...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            int n= ni();
            char[][] tanks= new char[n][];
            for (int i = 0; i < n; i++) tanks[i]= nw().toCharArray();

            int power= 0;
            String value= "1";
            for (int i = 0; i < n; i++) {
                int[] map= new int[10];
                for(char c: tanks[i]) map[c-'0']++;

                if(new String(tanks[i]).equals("0")) { out.println(0); return; }

                if(map[1]== 1 && map[0]== tanks[i].length-1) power+= map[0];
                else value= new String(tanks[i]);
            }

            out.print(value);
            for (int i = 0; i < power; i++) out.print(0);
            out.println();
        }
    }
