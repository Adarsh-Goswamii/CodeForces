...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            int n= ni(), v= ni();
            // new int{will_ripe_on, no_of_fruits}
            TreeSet<int[]> set= new TreeSet<>((i, j)-> (i[0]== j[0]? i[2]- j[2]: i[0]- j[0]));
            for (int i = 0; i < n; i++) set.add(new int[]{ni(), ni(), i});

            int edible= 0, prev= 0;
            while(!set.isEmpty()) {
                int curr_date= set.first()[0], ripe= 0;
                while(!set.isEmpty() && set.first()[0]== curr_date) ripe+= set.pollFirst()[1];
                ...
                if(!set.isEmpty() && curr_date+ 1== set.first()[0]) continue;
                else  {
                    edible+= Math.min(v, prev);
                    prev= 0;
                }
            }
