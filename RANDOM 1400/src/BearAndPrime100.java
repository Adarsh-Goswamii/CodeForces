...
public class BearAndPrime100 {
    ...
    void solve() throws Exception {
        int yes= 0, val= -1;
        print(2);
        if(ns().equals("yes")) { yes++; val= 2; }
        print(3);
        if(ns().equals("yes")) { yes++; val= 3; }
        print(5);
        if(ns().equals("yes")) { yes++; val= 5; }
        print(7);
        if(ns().equals("yes")) { yes++; val= 7; }

        if(yes>1) print("composite");
        else if(yes== 0) print("prime");
        else {
            Set<Integer> set= new HashSet<>(Arrays.asList(11, 13, 17, 19, 23, 29,
