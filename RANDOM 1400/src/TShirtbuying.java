...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            //  new int[]{cost, color_front, color_back};
            TreeSet<int[]>[] set= new TreeSet[4];
            for (int i = 0; i <= 3; i++) set[i]= new TreeSet<>((x, y)->(x[0]- y[0]));

            for (int i = 0; i < n; i++) {
                set[a[i]].add(new int[]{p[i], a[i], b[i]});
                set[b[i]].add(new int[]{p[i], a[i], b[i]});
            }

            int[] ans= new int[buyers]; Arrays.fill(ans, -1);
            for (int i = 0; i < buyers; i++) {
                if(set[color[i]].size()== 0) continue;
                int[] val= set[color[i]].first();
                set[val[1]].remove(val);
                set[val[2]].remove(val);
