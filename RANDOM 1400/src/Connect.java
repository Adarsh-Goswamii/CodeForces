...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            Map<Integer, Set<Integer>> mapi= new HashMap<>();
            Map<Integer, Set<Integer>> mapd= new HashMap<>();
            dfs(a, ir, ic, new boolean[n][n], mapi);
            dfs(a, dr, dc, new boolean[n][n], mapd);

//            out.println(mapi+" \n"+mapd);

            if(mapi.containsKey(dr) && mapi.get(dr).contains(dc)) out.println(0);
            else {
                long ans= imax;
                for(int ri: mapi.keySet()) {
                    for(int ci: mapi.get(ri)) {
                        ans= Math.min(ans, cost(ri, ci, mapd));
                    }
                }
    ...
    private long cost(int r, int ci, Map<Integer, Set<Integer>> map) {
        long ret= imax;
        for(int i: map.keySet()) for(int j: map.get(i))
            ret= Math.min(ret, (long)Math.pow(r-i, 2l)+ (long)Math.pow(ci-j, 2l));
        return ret;
    }

    private void dfs(char[][] a, int ir, int ic, boolean[][] v, Map<Integer, Set<Integer>> mapi) {
        if(ir== a.length || ic== a.length || ir<0 || ic<0 || v[ir][ic] || a[ir][ic]!= '0') return;
        else {
            v[ir][ic]= true;
            if(mapi.containsKey(ir)) mapi.get(ir).add(ic);
            else {
            ...
            dfs(a, ir+1, ic, v, mapi);
            dfs(a, ir, ic+1, v, mapi);
            dfs(a, ir-1, ic, v, mapi);
            dfs(a, ir, ic-1, v, mapi);
        }
    }
