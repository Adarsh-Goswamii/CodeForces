...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            List<Integer> list = new ArrayList<>(map.values()); Collections.sort(list);

            long ans= 0l;
            Set<Integer> set = new HashSet<>();
            for (int i = 0; i < n; i++) {
                if(set.contains(a[i])) continue;
                int ind= bin(list, i, n);
                set.add(a[i]);
                if(ind== -1) continue;
//                out.println(i+" "+a[i]+" "+(list.size()-ind));
                ans+= list.size()-ind;
            }
            out.println(ans);
        }
    ...
    // return the smallest value that is greater than find.
    private int bin(List<Integer> list, int find, int n) {
