...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for(int i: a) { map.put(i, map.getOrDefault(i, 0)+ 1); max= Math.max(max, map.get(i));}
            ...
            for(int i=0;i<n;i++) {
                if(map.get(a[i])== max) {
                    for (int j = i; j > 0; j--)
                        ...
                        else if(a[i]> a[j-1]) out.println(1+" "+(j)+" "+(j+1));
                        else out.println(2+" "+(j)+" "+(j+1));

                    for (int j = i; j < n-1; j++)
                        if(a[i]== a[j+1]) continue;
                        else if(a[i]> a[j+1]) out.println(1+" "+(j+2)+" "+(j+1));
                        else out.println(2+" "+(j+2)+" "+(j+1));
                    break;
                }
            }
        }
