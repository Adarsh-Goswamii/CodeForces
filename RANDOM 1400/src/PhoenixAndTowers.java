...
public class PhoenixAndTowers {
    ...
    void solve() throws Exception {
        ...
        for (int ii = 0; ii < t; ii++) {
            int n= ni(), m= ni(), x= ni();
            TreeSet<int[]> map= new TreeSet<>((a, b)-> (a[0]!= b[0]? a[0]-b[0]: a[1]- b[1]));
            for (int i = 0; i < n; i++) map.add(new int[]{ni(), i});

            TreeSet<int[]> tower= new TreeSet<>((a, b)-> (a[0]-b[0]!=0? a[0]-b[0]: a[1]-b[1]));
            for (int i = 0; i < m; i++) tower.add(new int[]{0, i+1});

            int[] ans= new int[n];
            while(!map.isEmpty()) {
                int[] val= map.pollLast();
                int[] val2= tower.pollFirst();
                val2[0]+= val[0];
                ans[val[1]]= val2[1];
            ...
            print(tower.last()[0]- tower.first()[0]<= x);
            print(ans);
        }
    }

    public static void main(String[] args) throws Exception {
        new PhoenixAndTowers().run();
    }
