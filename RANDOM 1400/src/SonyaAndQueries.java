...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for (int i = 0; i < q; i++) {
                ...
                if(s.charAt(0)== '?') {
                    s= s.substring(2);
                    String key= generatePattern(s);
                    out.println(map.getOrDefault(key, 0));
    ...
    private String generatePattern(String s) {
        char[] num= new char[19]; Arrays.fill(num, '0');

        int ind= s.length()-1, c= 18;
        while(ind!= -1) num[c--]= s.charAt(ind--);

        for (int i = 0; i < 19; i++) num[i]= (char)((num[i]-48)%2+ '0');
//        out.println(s+" "+new String(num));
        return new String(num);
    }

    public static void main(String[] args) throws Exception {
        new SonyaAndQueries().run();
    }
