...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            int[] p= new int[n+1];
            int[] c= new int[n+1];
            for (int i = 1; i <= n; i++) { p[i]= ni(); if(p[i]== -1) root= i; c[i]= ni(); }
            List<List<Integer>> adj= new ArrayList<>();
            for (int i = 0; i <= n; i++) adj.add(new ArrayList<>());
            for(int i=1;i<=n;i++) if(p[i]== -1) continue; else adj.get(p[i]).add(i);

            ans= new ArrayList<>();
            dfs(adj, c, root);
            Collections.sort(ans);

            if(ans.size()== 0) out.print(-1);
    ...
    private boolean dfs(List<List<Integer>> arr, int[] c, int root) {
        boolean respected= false;
        for(int i: arr.get(root)) {
            respected|= dfs(arr, c, i);
        }

        if(!respected && c[root]== 1) ans.add(root);
        return c[root]== 0;
    }
