...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            int min= 1;
            StringBuilder pre= new StringBuilder(s.charAt(0)+"");
            for(int i=1;i<n;i++) {
                pre.append(s.charAt(i));

                if(s.charAt((i)%min)< s.charAt(i)) break;
                else if(s.charAt((i)%min)> s.charAt(i)) min= i+1;
            }

            int val= (int)Math.ceil(k*1d/min);
            StringBuilder ans= new StringBuilder();
            String cand= s.substring(0, min);
            while(val--!= 0) ans.append(cand);
            out.println(ans.substring(0, k));
        }
    }
