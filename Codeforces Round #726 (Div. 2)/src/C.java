...
public class C {
    ...
    void solve() throws Exception {
        ...
        for (int ii = 0; ii < t; ii++) {
            ...
            sort(a);
            int min= imax, f= -1, l= -1;
            for (int i = 0; i < n-1; i++) {
                if(a[i+1]- a[i]< min) { f= i; l= i+1; min= a[i+1]- a[i]; }
            }

            List<Integer> list= new ArrayList<>();
            for (int i = 0; i < n; i++) if(i!= f && i!= l) list.add(a[i]);
            int index= bin(list, a[f]);

            out.print(a[f]+" ");
            for(int i= index;i<list.size();i++) out.print(list.get(i)+" ");
            for(int i=0;i<Math.min(list.size(), index);i++) out.print(list.get(i)+" ");
            out.println(a[l]);
        }
    }

    private int bin(List<Integer> list, int find) {
        int s= 0, l= list.size()-1, ret= list.size();
        while(s<= l) {
            int m= s+(l-s)/2;
