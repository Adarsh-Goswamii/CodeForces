...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for (int k = 1; k <= n; k++) {
                ...
                while (!q.isEmpty() && possible) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (ans[curr] != 0 && ans[curr] != assign) possible = false;
                        ans[curr] = assign;

                        for (int j : arr[curr]) {
                            if (ans[j] != 0 && ans[j] != assign) continue;
                            q.add(j);
                        }
                    }
                    assign = (assign == 1 ? 2 : 1);
            ...
            if (possible) {
                int one= 0, two= 0;
                for(int i: ans) if(i== 1) one++; else if(i== 2) two++;
                out.println(one);
                for (int i = 0; i <= n; i++) if(ans[i]== 1) out.print(i+" ");
                out.println("\n"+two);
                for (int i = 0; i <= n; i++) if(ans[i]== 2) out.print(i+" ");
                out.println();
            } else {
                out.println(-1);
            }
