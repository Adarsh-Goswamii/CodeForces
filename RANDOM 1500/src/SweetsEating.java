...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            int n= ni(), m= ni();
            int[] a= ni(n); sort(a, Collections.reverseOrder());

            long[] prefix= new long[n];
            for (int i = n-1; i >= 0; i--) prefix[i]= (i!= n-1? prefix[i+1]: 0)+ a[i];

            long[] dp= new long[n+m];
            for (int i = n-1; i >= 0; i--) dp[i]= dp[i+m]+ prefix[i];

            for(int i= n-1;i>= 0;i--) out.print(dp[i]+" ");
            out.println();
        }
    }

    public static void main(String[] args) throws Exception {
        new SweetsEating().run();
    }
    ...
    private void sort(int[] arr, Comparator comp) {
        List<Integer> list = new ArrayList<>();
        for (int i : arr) list.add(i);
        if(comp!= null) Collections.sort(list, comp);
