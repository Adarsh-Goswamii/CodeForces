...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            for (int i = 0; i < n; i++) {
                total[arr[i] - 'a']++;
                for (int j = 0; j < 26; j++) pre[i][j] = total[j];
            }

            total = new int[26];
            for (int i = n - 1; i >= 0; i--) {
                total[arr[i] - 'a']++;
                for (int j = 0; j < 26; j++) suff[i][j] = total[j];
            }
            ...
            for (int i = 0; i < q; i++) {
                ...
                int[] ans = new int[26];
                for (int j = 0; j < 26; j++) ans[j] = total[j];
                if (s != -1) {
                    for (int j = 0; j < 26; j++) ans[j] -= pre[s][j];
                }

                if (l != n) {
                    for (int j = 0; j < 26; j++) ans[j] -= suff[l][j];
                }
...
//                print(ans);
                long ret = 0l;
                for (int j = 0; j < 26; j++) ret += ans[j] * (j + 1);
                out.println(ret);
            }
        }
    }
