...
//        out.println(count);
        ...
        for (int ii = 0; ii < t; ii++) {
            int a= ni(), b= ni(), c= ni(), x= -1, y= -1;

            List<Integer> prime = new ArrayList<>(Arrays.asList(
                    2, 3, 11, 13, 101, 103, 1009, 1013, 10007,
                    10009, 100003, 100019, 1000003, 1000033, 10000019, 10000169, 100050001, 100030001));

            int gcd= prime.get(2*(c-1));
            for(int i: prime) if(((i*gcd)+"").length()== a) { x= i; break; }
            for(int i: prime) if(((i*gcd)+"").length()== b && x!= i) { y= i; break; }
            int l1= (x*gcd+"").length(), l2= (y*gcd+"").length();
            long gcd2= gcd(gcd*x, gcd*y);

            out.println(x*gcd +" "+ y*gcd);
//            out.println(l1+" "+l2);
//            out.println(gcd2+" "+gcd);

//            if(l1!= a || l2!= b || gcd2!= gcd)
//            out.println(a+" "+b+" "+c);
        }
