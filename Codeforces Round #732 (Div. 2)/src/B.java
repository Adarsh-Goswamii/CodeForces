...
public class B {
    ...
    void solve() throws Exception {
        ...
        for (int ii = 0; ii < t; ii++) {
            ...
            char[][] ch= new char[n-1][m];
            for (int i = 0; i < n-1; i++) ch[i]= ns().toCharArray();

            int[][] map= new int[m][26];
            for (int i = 0; i < n; i++) for (int j = 0; j < m; j++) map[j][arr[i][j]-'a']++;
            for (int i = 0; i < n-1; i++) for (int j = 0; j < m; j++) map[j][ch[i][j]-'a']--;

            char[] ans= new char[m];
            for (int i = 0; i < m; i++) for (int j = 0; j < 26; j++) if(map[i][j]==1) ans[i]= (char)(j+'a');
