    ...
    void solve() throws Exception {
        ...
        for (int ii = 0; ii < t; ii++) {
            ...
            if(sum!= 0) out.println("-1");
                ...
                for(int i=0;i<n;i++) {
                    if(a[i]> b[i]) g.add(i+1);
                    else if(a[i]< b[i]) s.add(i+1);
                }
...
//                    System.out.println(g+"\n"+s);
                    int val_g= g.get(g.size()-1), val_s= s.get(s.size()-1);
                    int oper= Math.min(Math.abs(b[val_g-1]- a[val_g-1]), Math.abs(b[val_s-1]- a[val_s-1]));
//                    System.out.println("oper "+ oper);
                    count+= oper;
                    for (int i = 0; i < oper; i++) ans.append(val_g+" "+val_s+"\n");

                    if(Math.abs(b[val_g-1]- a[val_g-1])== oper) g.remove(g.size()-1);
                    else a[val_g-1]-= oper;

                    if(Math.abs(b[val_s-1]- a[val_s-1])== oper) s.remove(s.size()-1);
                    else a[val_s-1]+= oper;
                }
