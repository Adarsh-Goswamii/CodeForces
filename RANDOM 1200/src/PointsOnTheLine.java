...
//        t = ni();
        ...
        for (int ii = 0; ii < t; ii++) {
            ...
            int ans= n;
            for(int i=0;i<n;i++) {
                int index= binarySearch(arr, i, n-1, arr[i]+ d);

                ans= Math.min(ans, i+n-index-1);
            }
    ...
    private int binarySearch(int[] arr, int start, int last, int find) {
        int ret= start;
        while(start<= last) {
