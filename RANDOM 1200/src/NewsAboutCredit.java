...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            int n= ni(), sum= 0;
            int[][] arr= new int[n][2];
            for (int i = 0; i < n; i++) { arr[i]= new int[]{ni(), i+1}; sum+= arr[i][0]; }

            if(arr[0][0]== 0 || sum< n-1) out.println(-1);
            else {
                List<int[]> list = new ArrayList<>();
                for(int i=1;i<n;i++) list.add(arr[i]);
                ...
                int i= -1, j= 0;
                while(j< n-1) {
                    int val= i== -1? arr[0][0]: list.get(i)[0], person= i==-1? 1: list.get(i)[1];
                    while(j<n-1 && val--!= 0) {
                        count++;
                        ans.append(person+" "+list.get(j)[1]+"\n");
                        j++;
                    }
                    i++;
                }
