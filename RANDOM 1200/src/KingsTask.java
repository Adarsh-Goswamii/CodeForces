...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            int ans= Math.min(helper(arr, n, 0), helper(arr, n, 1));

            if(ans!= imax) out.println(ans);
            else out.println(-1);
        }
    }

    private int helper(List<Integer> _arr, int n, int start) {
        List<Integer> arr = new ArrayList<>(_arr);
        HashSet<List<Integer>> set= new HashSet<>();
        for(int i=start;;i++) {
            if(set.contains(arr)) break;
            set.add(arr);

            if(sorted(arr)) return i- start;

            if(i%2== 0) {
                for (int j = 0; j < n/2; j++) {
                    int temp= arr.get(n/2+j);
                    arr.set(n/2+j, arr.get(j));
                    arr.set(j, temp);
                }
            }
            else {
    ...
    private boolean sorted(List<Integer> arr) {
        ...
        for (int i = 0; i < arr.size(); i++) if(arr.get(i)!= temp.get(i)) return false;
        return true;
    }
    ...
    static void stack_l(int[] arr, int[] left) {
    ...
    static void stack_r(int[] arr, int[] right) {
        Stack<Integer> stack = new Stack<>();
