...
//        t = ni();
        for (int ii = 0; ii < t; ii++) {
            ...
            int n= ni();
            char[][] l= new char[n][];
            for (int i = 0; i < n; i++) l[i]= ns().toCharArray();

            boolean ans= true;
            for (int i = 0; i < n; i++) if(same(arr, l[i])) ans= false;

            print(ans);
        }
    }

    private boolean same(char[] arr, char[] ch) {
        if(arr.length!= ch.length) return false;

        boolean same= true;
        Set<Character> set1 = new HashSet<>(Arrays.asList('O', '0', 'o'));
        Set<Character> set2 = new HashSet<>(Arrays.asList('1', 'l', 'I', 'i', 'L'));
        for(int i=0;i<arr.length;i++) {
            if(arr[i]!= ch[i]) {
                ...
                if(c-32!= c2 && c+32!= c2)
                    if(!set1.contains(c) || !set1.contains(c2))
                        if(!set2.contains(c) || !set2.contains(c2))
                            same= false;
            }
        }

//        out.println(new String(arr)+" "+new String(ch)+" "+same);
        return same;
    }

    public static void main(String[] args) throws Exception {
        new LoginVerification().run();
    }
